...
import datetime  
from src.dataloader_td import data_loader
from src.utils import update_plot_td,save_confusion_matrix_valence3,save_tSNE_valence3
...
class ModelTraining:
    ...
    def train_begin(self):
        ...
        # Load data for data
        train_loader, valid_loader,test_loader=data_loader(self.args)
        # Start training process
        self.train_model(settings, hyp_params, train_loader, valid_loader,test_loader)

    def train_model(self,settings, hyp_params, train_loader,valid_loader,  test_loader):
        ...
        with open(hyp_params.logs_path+'/'+hyp_params.dataset+'_'+hyp_params.data_index+hyp_params.exp_number+'.txt', 'a') as log_file:
            for epoch in range(1, hyp_params.num_epochs+1): 
                ...
                valid_accuracy,valid_precision,valid_recall,valid_f1,valid_loss,valid_epoch_time,valid_predict_data,valid_true_data,valid_last_features = self.evaluate(hyp_params,model, criterion, valid_loader, test_loader, test=False)
                test_accuracy,test_precision,test_recall,test_f1,test_loss,test_epoch_time,test_predict_data,test_true_data,test_last_features = self.evaluate(hyp_params,model, criterion, valid_loader, test_loader, test=True)
                ...
                update_plot_td(epoch, train_loss_list,valid_loss_list,test_loss_list,train_accuracy_list,valid_accuracy_list,test_accuracy_list, 
                train_precision_list,valid_precision_list,test_precision_list,train_recall_list,valid_recall_list,test_recall_list,
                train_f1_list,valid_f1_list,test_f1_list,hyp_params.pictures_path+'/'+'_'+hyp_params.dataset+'_'+hyp_params.data_index+hyp_params.exp_number)
                ...
                valid_log_message = f'{current_time} - Epoch [{epoch}/{hyp_params.num_epochs}], Valid_loss: {valid_loss:.4f}, '\
                f'Valid_ac: {valid_accuracy:.4f},Valid_pr: {valid_precision:.4f},Valid_re: {valid_recall:.4f}, Valid_f1: {valid_f1:.4f}, Valid_time: {valid_epoch_time:.4f}s'
                ...
                if valid_accuracy > best_valid_accuracy and valid_accuracy > hyp_params.accuracy_threshold: 
                    
                    print(f"{hyp_params.models_path}/{hyp_params.model+hyp_params.data_index}.pt!")
                    save_model(hyp_params, model, name) 
                    save_best_result_td(hyp_params,train_log_message,valid_log_message,test_log_message,name) 
            ...
            save_confusion_matrix_valence3(hyp_params,predict_data,true_data,name) 
            last_features = load_last_features(hyp_params,name)
            save_tSNE_valence3(hyp_params,last_features,true_data,name)
    ...
    def evaluate(self,hyp_params,model, criterion, valid_loader, test_loader, test=False):
        """Evaluation function"""
        model.eval()
